...
class Media(models.Model):
    ...
    add_date = models.DateTimeField("Date Produced", blank=True, db_index=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.uploaded_poster and self.uploaded_poster != self.__original_uploaded_poster:
            with open(self.uploaded_poster.path, "rb") as f:
                ...
                myfile = File(f)
                thumbnail_name = helpers.get_file_name(
                    self.uploaded_poster.path)
                self.uploaded_thumbnail.save(
                    content=myfile, name=thumbnail_name)
    ...
    def set_media_type(self, save=True):
        ...
        if self.media_type in ["audio", "image", "pdf"]:
            ...
            if ret.get("is_audio") or audio_file_with_thumb:
                ...
                self.duration = int(
                    float(ret.get("audio_info", {}).get("duration", 0)))
                self.encoding_status = "success"
                ...
                if profile.extension == "gif":
                    ...
                    tasks.encode_media.apply_async(
                        args=[self.friendly_token,
                              profile.id, encoding.id, enc_url],
                        kwargs={"force": force},
                        priority=0,
                    )
            profiles = [p.id for p in profiles]
            tasks.chunkize_media.delay(
                self.friendly_token, profiles, force=force)
        else:
            for profile in profiles:
                if profile.extension != "gif":
    ...
    def set_encoding_status(self):
        ...
        """
        mp4_statuses = set(encoding.status for encoding in self.encodings.filter(
            profile__extension="mp4", chunk=False))
        webm_statuses = set(encoding.status for encoding in self.encodings.filter(
            profile__extension="webm", chunk=False))
    ...
    def encodings_info(self, full=False):
        ...
        if full:
            ...
            for codec in extra:
                ...
                v = self.encodings.filter(
                    chunk=True, profile__codec=codec).values("progress")
                ret[resolution][codec]["progress"] = sum(
                    [p["progress"] for p in v]) / v.count()
                ...
                if os.path.exists(hls_file):
                    ...
                    for iframe_playlist in m3u8_obj.iframe_playlists:
                        ...
                        if os.path.exists(uri):
                            ...
                            res["{}_iframe".format(
                                resolution)] = helpers.url_from_path(uri)
...
class Category(models.Model):
    ...
    def update_category_media(self):
        ...
        self.media_count = Media.objects.filter(
            listable=True, category=self).count()
    ...
    def thumbnail_url(self):
        ...
        media = Media.objects.filter(
            category=self, state="public").order_by("-views").first()
        if media:
...
class Tag(models.Model):
    ...
    def update_tag_media(self):
        self.media_count = Media.objects.filter(
            state="public", is_reviewed=True, tags=self).count()
        self.save(update_fields=["media_count"])
        return True
    ...
    def thumbnail_url(self):
        if self.listings_thumbnail:
            return self.listings_thumbnail
        media = Media.objects.filter(
            tags=self, state="public").order_by("-views").first()
        if media:
            return media.thumbnail_url
...
class EncodeProfile(models.Model):
    ...
    resolution = models.IntegerField(
        choices=ENCODE_RESOLUTIONS, blank=True, null=True)

    codec = models.CharField(
        max_length=10, choices=CODECS, blank=True, null=True)
...
class Encoding(models.Model):
    ...
    chunk = models.BooleanField(
        default=False, db_index=True, help_text="is chunk?")
    ...
    media = models.ForeignKey(
        Media, on_delete=models.CASCADE, related_name="encodings")
...
class Subtitle(models.Model):
    ...
    media = models.ForeignKey(
        Media, on_delete=models.CASCADE, related_name="subtitles")
...
class Rating(models.Model):
    ...
    media = models.ForeignKey(
        Media, on_delete=models.CASCADE, related_name="ratings")

    rating_category = models.ForeignKey(
        RatingCategory, on_delete=models.CASCADE)
...
class Comment(MPTTModel):
    ...
    user = models.ForeignKey(
        "users.User", on_delete=models.CASCADE, db_index=True)
...
def media_save(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        instance.media_init()
        notify_users(friendly_token=instance.friendly_token,
                     action="media_added")
...
def encoding_file_save(sender, instance, created, **kwargs):
    ...
    if instance.chunk and instance.status == "success":
        ...
        if instance.media_file:
            ...
            if complete:
                ...
                with tempfile.TemporaryDirectory(dir=settings.TEMP_DIRECTORY) as temp_dir:
                    seg_file = helpers.create_temp_file(
                        suffix=".txt", dir=temp_dir)
                    tf = helpers.create_temp_file(suffix=".{0}".format(
                        instance.profile.extension), dir=temp_dir)
                    ...
                    with open(tf, "rb") as f:
                        ...
                        output_name = "{0}.{1}".format(
                            helpers.get_file_name(
                                instance.media.media_file.path),
                            instance.profile.extension,
                        )
                        encoding.media_file.save(
                            content=myfile, name=output_name)
                    ...
                    if (
                        ...
                        who = Encoding.objects.filter(
                            media=encoding.media, profile=encoding.profile).exclude(id=encoding.id)
                        who.delete()
                    else:
                        encoding.delete()
                    ...
                    instance.media.post_encode_actions(
                        encoding=instance, action="add")
        ...
        chunks = Encoding.objects.filter(
            media=instance.media, chunks_info=instance.chunks_info, chunk=True).order_by("add_date")
        ...
        encodings = set(
            [encoding.status for encoding in Encoding.objects.filter(media=instance.media)])
...
def encoding_file_delete(sender, instance, **kwargs):
    ...
    if instance.media_file:
        ...
        if not instance.chunk:
            instance.media.post_encode_actions(
                encoding=instance, action="delete")
