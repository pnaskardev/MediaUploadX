...
def pre_save_action(media, user, session_key, action, remote_ip):
    ...
    if user:
        query = MediaAction.objects.filter(
            media=media, action=action, user=user)
    else:
        query = MediaAction.objects.filter(
    ...
    if not user:
        ...
        query = MediaAction.objects.filter(media=media, action=action, remote_ip=remote_ip).filter(
            user=None).order_by("-action_date")
        if query:
            query = query.first()
            now = datetime.now(query.action_date.tzinfo)
            if action == "watch":
...
URL: %s
    ...
    for item in notify_items:
        email = EmailMessage(item["title"], item["msg"],
                             settings.DEFAULT_FROM_EMAIL, item["to"])
...
def show_recommended_media(request, limit=100):
    ...
    if pmi:
        media = list(models.Media.objects.filter(friendly_token__in=pmi).filter(
            basic_query).prefetch_related("user")[:limit])
    else:
        media = list(models.Media.objects.filter(basic_query).order_by(
            "-views", "-likes").prefetch_related("user")[:limit])
...
def show_related_media_content(media, request, limit):
    ...
    m = list(models.Media.objects.filter(
        q_author).order_by().prefetch_related("user")[:limit])
    ...
    if len(m) < limit:
        ...
        if category:
            ...
            q_res = models.Media.objects.filter(q_category).order_by(order_criteria[random.randint(
                0, len(order_criteria) - 1)]).prefetch_related("user")[: limit - media.user.media_count]
            m = list(itertools.chain(m, q_res))

        if len(m) < limit:
            q_generic = Q(listable=True)
            q_res = models.Media.objects.filter(q_generic).order_by(order_criteria[random.randint(
                0, len(order_criteria) - 1)]).prefetch_related("user")[: limit - media.user.media_count]
            m = list(itertools.chain(m, q_res))
...
def update_user_ratings(user, media, user_ratings):
    ...
    for rating in user_ratings:
        user_rating = models.Rating.objects.filter(
            user=user, media_id=media, rating_category_id=rating.get("category_id")).only("score").first()
        if user_rating:
...
def notify_user_on_mention(friendly_token, user_mentioned, cleaned_comment):
    ...
    if user.notification_on_comments:
        title = "[{}] - You were mentioned in a comment".format(
...
Comment : %s
        ...
        email = EmailMessage(
            title, msg, settings.DEFAULT_FROM_EMAIL, [user.email])
        email.send(fail_silently=True)
    return True
